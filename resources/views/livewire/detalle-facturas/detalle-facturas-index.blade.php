<div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">
    <h1 class="text-xl font-semibold">Detalles de Facturas</h1>

    <div class="overflow-x-auto rounded-xl border border-neutral-200 dark:border-neutral-700">
        <table class="min-w-full divide-y divide-neutral-200 dark:divide-neutral-700">
            <thead class="bg-neutral-100 dark:bg-neutral-800">
                <tr>
                    <th class="px-6 py-3 text-left text-sm font-medium text-neutral-700 dark:text-neutral-300">No. Factura</th>
                    <th class="px-6 py-3 text-left text-sm font-medium text-neutral-700 dark:text-neutral-300">Producto</th>
                    <th class="px-6 py-3 text-left text-sm font-medium text-neutral-700 dark:text-neutral-300">Cantidad</th>
                    <th class="px-6 py-3 text-left text-sm font-medium text-neutral-700 dark:text-neutral-300">V. Unitario</th>
            ...
            <tbody class="bg-white dark:bg-neutral-900 divide-y divide-neutral-200 dark:divide-neutral-800">
                @foreach ($detalles as $detalle)
                    <tr>
                        ...
                        <td class="px-6 py-4 text-sm text-neutral-800 dark:text-neutral-200">
                            ${{ number_format($detalle->Vunitario, 2) }}
                        </td>
                        <td class="px-6 py-4 font-semibold text-sm text-neutral-800 dark:text-neutral-200">
                            ${{ number_format($detalle->valorTotal, 2) }}
                        </td>
                        <td class="px-6 py-4 text-sm text-right text-neutral-800 dark:text-neutral-200">
                            <div class="flex justify-end gap-2">
                                <a href="{{ route('detalles-factura.editar', $detalle->id) }}">
                                    <x-button
                                        color="yellow"
                                        size="xs"
                                        icon="pencil"
    ...
    <div class="w-full text-right mt-2">
        <a href="{{ route('detalles-factura.crear') }}">
            <button
